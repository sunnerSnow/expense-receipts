...
6. **發票號碼格式是 2 個英文字母 + 8 碼數字**(如 AB12345678)。收據沒有發票號碼,回空字串。
7. currency 用 ISO 4217 代碼,台灣單據就是 TWD。
...
/** 發票號碼:去空白與連字號、轉大寫,須為 2 英文字母 + 8 數字 */
...
export function normalizeRecognition(
  ...
  const invoiceNumber = invoiceNumberRaw === "" ? null : normalizeInvoiceNumber(invoiceNumberRaw);
  if (invoiceNumberRaw !== "" && invoiceNumber === null) {
    warnings.push(`發票號碼格式不正確(「${invoiceNumberRaw}」),已略去`);
  ...
  const sellerTaxId = sellerTaxIdRaw === "" ? null : normalizeTaxId(sellerTaxIdRaw);
  if (sellerTaxIdRaw !== "" && sellerTaxId === null) {
    warnings.push(`賣方統編不是 8 碼數字(「${sellerTaxIdRaw}」),已略去`);
  }

  const buyerTaxIdRaw = field(obj, "buyerTaxId");
  const buyerTaxId = buyerTaxIdRaw === "" ? null : normalizeTaxId(buyerTaxIdRaw);
  if (buyerTaxIdRaw !== "" && buyerTaxId === null) {
    warnings.push(`買方統編不是 8 碼數字(「${buyerTaxIdRaw}」),已略去`);
  }
  if (buyerTaxId !== null && buyerTaxId !== options.companyTaxId) {
