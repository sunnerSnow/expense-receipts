...
| 手開發票、收銀機發票、收據、國外單據 | Claude API vision + structured outputs(Phase 2) | 每張約 NT$0.6,結果進「待確認」 |
...
```bash
pnpm install
cp .env.example .env          # 填入實際值(至少改 COMPANY_TAX_ID)
docker compose up -d          # PostgreSQL 17,host port 5433(避開 booking-crm 的 5432)
pnpm db:migrate               # 套用 migrations
pnpm --filter @expense-receipts/db seed   # 預設分類 + admin 使用者
pnpm dev                      # Next.js dev server(http://localhost:3000)
pnpm worker                   # 背景工作程序(另開終端;Phase 2 起才需要常駐)
```
...
```
apps/web          Next.js 15 PWA:拍照上傳、QR 解析(客戶端)、待確認佇列、分類、報表
apps/worker       pg-boss:AI 辨識(Phase 2)、匯出檔產生(Phase 3)
packages/core     業務邏輯(純函式,禁止 IO):QR 解析、扣抵判斷、狀態機、預設分類
packages/db       Drizzle schema + migrations + seed
packages/config   zod 環境變數驗證(parseEnv)
```
...
拍照 ──▶ 客戶端 jsQR 解碼
          ...
          └─ 解不到 ──▶ 上傳影像 ──▶ worker 丟 Claude API 辨識(Phase 2)
                                        └──▶ 待確認(pending_review)──人工確認──▶ confirmed
...
- **Phase 2 — AI 辨識**:Claude API 辨識非電子發票單據、統編比對、自動分類建議、待確認流程
