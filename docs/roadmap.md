...
- ✅ **檔案位置可搬家**(2026-08-06,ADR-0007):寫入端存相對鍵值,讀取端把
  舊的絕對路徑正規化 —— 換機器、換使用者名稱、搬進容器、搬上 VPS 都不會讓
  憑證影像失聯,而且**一列資料都不用改**(鐵律 5 不必開例外)。
...
- ✅ **PWA 離線佇列**(2026-09-21,ADR-0008):人在國外、電腦沒開時,
  `/receipts/new/offline` 照樣打得開(service worker),拍照 + 寫備註存進手機的
  IndexedDB,等連得到伺服器、打開 App 就自動補送並進 AI 辨識。
  連線判斷改問 `/api/ping` 而不是看 `navigator.onLine` —— 最常見的情境
  「手機有 4G、辦公室電腦沒開」在 onLine 眼中是「有網路」。
  補送以客戶端 UUID 當冪等鍵,不會因為重送變成兩筆憑證。
  實測:停掉 web 容器 → 頁面仍打得開 → 存進佇列 → 容器回來 → 自動補送 →
  單據落在 pending_review
